...
/*
    //Задача 2.
     public static void main(String[] args) {
        ...
        List<String> uppercaseNames = names.stream().filter(name -> !name.isEmpty()).map(String::toUpperCase).collect(Collectors.toList());

        System.out.println("Непустые имена в верхнем регистре: " + uppercaseNames);


        char targetLetter = 'P';
        long count = names.stream().filter(name -> !name.isEmpty() && name.charAt(0) == targetLetter).count();
